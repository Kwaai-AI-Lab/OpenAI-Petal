...
def fetch_health_state(dht: hivemind.DHT) -> dict:
    ...
    if model_index is not None and isinstance(model_index.value, dict):
        ...
        models.extend(sorted(custom_models, key=lambda info: (-info.num_blocks, str(info.dht_prefix))))
