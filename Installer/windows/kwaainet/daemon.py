...
class DaemonProcess:
    ...
    def daemonize(self):
        """Create daemon process (Windows-compatible)"""
        import platform
        
        if platform.system() == "Windows":
            # Windows doesn't support fork, so we simulate daemon behavior
            # by detaching from console and redirecting I/O
            try:
                import subprocess
                import sys
                ...
                # Redirect standard file descriptors to suppress console output
                sys.stdout.flush()
                sys.stderr.flush()
                
                # On Windows, we'll run as a detached process
                logger.info(f"Windows daemon started with PID {pid}")
                return pid
                
            except Exception as e:
                logger.error(f"Windows daemon creation failed: {e}")
                sys.exit(1)
        else:
            # Unix/Linux fork-based daemon
            try:
                # First fork
                pid = os.fork()
                if pid > 0:
